...
def data_color(
    ...
    for col in columns_resolved:
        ...
        filtered_column_vals = column_vals.filter(~_filter)
        ...
        color_vals = [na_color if is_na(data_table.to_native(), x) else x for x in color_vals]
    ...
    if domain_range == 0:
        ...
        if truncate:
            ...
            nan_vals: SeriesT = nw.new_series(
                "nan", values=[float("nan")] * len(vals), backend=impl
            )
            scaled_vals = scaled.zip_with((scaled.is_between(0, 1, closed="both")), nan_vals)
    return scaled_vals


def _rescale_factor(vals: SeriesT, domain: list[float], palette: list[str]) -> SeriesT:
    """
...
def _get_domain_numeric(vals: nw.Series[Any]) -> list[float | int]:
    """
    Get the domain of numeric values.

    Get the domain of numeric values in `vals=` as a tuple of two values: the min and max values.
    """
